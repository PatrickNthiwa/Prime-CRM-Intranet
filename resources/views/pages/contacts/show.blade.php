...
@section('content')
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{{ url('/admin/contacts') }}">Your Contacts</a></li>
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                @if(\Auth::user()->is_admin == 1)
                                    <tr>
                                        <th>Contact Details</th><td></td>
                                    </tr>
                                @endif
                                <tr><th> Name </th><td> {{ $contact->first_name }} {{ $contact->last_name }} </td>
                               ...
                               @if($contact->documents->count() > 0)
                                ...
                                @endif
                                <tr><th> Branch </th><td> {{ $contact->referral_cource }} </td></tr>
                                <!--<tr><th> Position title </th><td> {{ $contact->position_title }} </td></tr>-->
                                <!--<tr><th> Industry </th><td> {{ $contact->industry }} </td></tr>-->
                                <tr><th> Product type </th><td> {{ $contact->project_type }} </td></tr>
                                <!--<tr><th> Description </th><td> {{ $contact->project_description }} </td></tr>-->
                                ...
                                <tr><th> Area | Location </th><td> {{ $contact->address_city }} </td></tr>
                                <tr><th> County </th><td> {{ $contact->address_state }} </td></tr>
                                ...
                                <tr><th> Created on </th><td>{{ $contact->created_at }}</td></tr>
                                 <tr><th> Status </th><td> <i class="">{{ $contact->getStatus->name }}</i> </td></tr>
                                <!--<tr><th> Modified at </th><td>{{ $contact->updated_at }}</td></tr>-->
                                  <tr><th>Short Notes </th><td>  <textarea class="form-control" name="description" type="text" id="description"rows="10" cols="50"> {{ $contact->description }} </textarea></td></tr>
